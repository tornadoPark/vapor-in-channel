...
其中$C$是通道截面周长。在具体计算中，积分被写成求和（数值积分）：
$$
...
+ 导热方程的初始条件：
  ...
  \cdot
  \sech\left(\sqrt\frac{h\cdot C_{heat}}{k_s\cdot A_{heat}}l_{tot}\right)
  $$
  ...
  + Cheat和Aheat分别是参与肋片换热的固体的长度和截面积，因此$C_{heat}=C$
...
求解流程![未命名文件 (1)](考虑气体温度变化的变壁面温度等液膜厚度带排气的模型.assets/未命名文件 (1).jpg)
